...
export default function TradesPage() {
  ...
  const [closeModal, setCloseModal] = useState<{ id: string; direction: 'LONG' | 'SHORT'; entry: number; tp1: number; tp2: number; sl: number } | null>(null);
  ...
  // Auto-fill exit price based on result selection
      ...
      <div className="px-4 pt-14 pb-3 safe-top border-b border-[#1E1E2E]">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-[#606080] text-xs mt-0.5">{trades.length} 筆紀錄</p>
          </div>
          <button
            onClick={exportCsv}
            className="text-[#F0B90B] text-xs font-semibold px-3 py-1.5 border border-[#F0B90B]/40 rounded-full active:opacity-70"
          >
            匯出 CSV
          </button>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <StatCard label="總交易" value={String(closed.length)} sub={`${pending.length} 持倉中`} />
          <StatCard label="勝率" value={winRate !== null ? `${winRate}%` : '—'} sub={`${wins.length}W / ${closed.length - wins.length}L`} color={winRate !== null ? (winRate >= 50 ? '#00C851' : '#FF4444') : undefined} />
          <StatCard label="平均損益" value={avgPnl !== null ? `${avgPnl}%` : '—'} sub="每筆平均" color={avgPnl !== null ? (parseFloat(avgPnl) >= 0 ? '#00C851' : '#FF4444') : undefined} />
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-3 scroll-container">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 gap-2 text-center">
            ...
            <p className="text-[#606080] text-sm">當 3 星信號出現時會自動新增</p>
          </div>
        ) : (
          filtered.map((trade) => {
            ...
            return (
              <div key={trade.id} className="bg-[#12121A] border border-[#1E1E2E] rounded-2xl p-4 mb-3">
                {/* Top row */}
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    {isPending ? (
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: `${RESULT_COLOR[trade.result!]}20`, color: RESULT_COLOR[trade.result!] }}>
                ...
                {!isPending && trade.exitPrice !== undefined && (
                  <div className="flex items-center justify-between mt-1 pt-2 border-t border-[#1E1E2E]">
                    <span className="text-[#606080] text-xs">出場價 ${fmtPrice(trade.exitPrice)}</span>
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <div className="flex gap-2">
                    {isPending && (
                      <button
                        onClick={() => setCloseModal({ id: trade.id, direction: trade.direction, entry: trade.entry, tp1: trade.tp1, tp2: trade.tp2, sl: trade.stopLoss })}
      ...
      {closeModal && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-end" onClick={(e) => e.target === e.currentTarget && setCloseModal(null)}>
          <div className="w-full max-w-xl mx-auto bg-[#12121A] rounded-t-3xl p-6 pb-10 border-t border-[#1E1E2E]">
            ...
            <h2 className="text-[#EAEAF4] text-lg font-extrabold mb-4">記錄交易結果</h2>

            <p className="text-[#606080] text-xs mb-2">選擇結果（會自動填入對應出場價）</p>
...
function StatCard({ label, value, sub, color }: { label: string; value: string; sub: string; color?: string }) {
  return (
    <div className="bg-[#12121A] border border-[#1E1E2E] rounded-2xl p-3 text-center">
      <p className="text-[#606080] text-xs mb-1">{label}</p>
      <p className="font-extrabold text-lg" style={{ color: color ?? '#EAEAF4' }}>{value}</p>
      <p className="text-[#404060] text-xs">{sub}</p>
    </div>
  );
}
