...
## ⏰ 2026-08-30 前後要做一次：檢查 Vercel CPU 走勢

**背景**：8/26 Vercel Fluid Active CPU 實測 **4h5m / 4h（102%）**，是唯一超標的
項目（其餘全部 < 10%：呼叫次數 78K/1M、流量 373MB/100GB）。額度週期
7/27–8/26，**8/27 重置**。

換算下來每輪掃描的預算只有約 **1.7 CPU 秒**——這個系統一直貼著上限跑，
從來沒有餘裕。

**8/26 已做的兩個修正**（都不動策略邏輯）：
- 掃描前剔除「幣安端下不了單」的 symbol（`240352a`）。原本一個 -4141 的幣會
  每輪重發，白燒一次完整指標計算加一則推播。省約 1/15。
- 悲觀影子軌跡改成預設關閉（`SHADOW_PESSIMISTIC=1` 才開）。每筆影子單少跑
  一次 walkTpSl（96-168 根 K 線）。

**為什麼不直接改 cron**：預估修正後落在 80~90%，餘裕只有 10~20%，但沒有把握。
而且有個反方向因素——上個週期最後 3 天 Redis 是死的、影子模擬完全沒跑，
**那幾天的 CPU 偏低**；Redis 9/1 回來後影子會跑滿整個月，下期基準線可能更高。
所以用量測決定，不用猜。
