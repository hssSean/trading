...
### `DEFAULT_MAX_DRAWDOWN_R` 訂得太緊——8R 有 58% 機率被純雜訊觸發

`npx tsx scripts/drawdown-threshold.ts` 用真實 R 序列（n=69，sd=1.29）做
bootstrap：**假設策略期望值 = 0**，每 50 筆一段，最大回撤的分布是
...
門檻  8R → 純雜訊觸發率 57.7%
門檻 10R → 39.0%
門檻 12R → 24.9%   ← 目前設定（route.ts:1840）
```

**原本的 8R 不是失效偵測器，是雜訊偵測器。** 這解釋了為什麼回撤停機一直觸發。

去均值是關鍵：觀測到的平均是 −0.107R，直接用會把「策略確實在虧」也算進
「正常波動」，門檻就會被訂得過寬（照觀測分布 p95 是 22.1R）。去掉漂移只留
波動，問的才是「純雜訊能造成多大回撤」。

**建議值 p95 ≈ 18R**——每 20 段誤觸發 1 次。誤觸發的代價只是「人看一眼按一顆
按鈕」（解除按鈕已於 `82b003f` 補上），漏擋的代價是繼續虧，兩邊不對稱。

⚠ **這是風控門檻的放寬，需要使用者明確決定，不要自己改。** 而且 n 還小
（檢定力邊界 |mean R| > 0.31R），累積更多乾淨交易後要重跑。
...
## 2026-09-03：TP 位置量測完畢 → 最後一個未測參數家族也是雜訊

**這關掉了「還有哪裡沒試過」這個問題。**

先前 10 個出場變體全部只動**出場管理**（保本點、移動止損倍數、停滯、到期、
TP1 比例），沒有一個動到 **TP 位置本身**——而位置直接決定勝率（目標越遠越難
達到），實測缺的正是勝率（26.3% vs 兩平所需 30.8%）。

`exitPolicy.ts` 新增 `tp1AtR` / `tp2AtR`，可以把 TP 移到「進場價 ± 風險 ×
倍數」重跑歷史。線上是 TP1 +2R / TP2 +3.5R（`signals.ts` buildSignalLevels）。

小樣本看起來有發現，**大樣本推翻**：

```
n=200（3個月×8幣）   TP1 推遠到 +2.5R   每筆 +0.029R   t=3.41  ★
n=705（6個月×15幣）  TP1 推遠到 +2.5R   每筆 +0.014R   t=2.13
```

效果量減半、t 下降。若效應為真，t 應隨 √3.5 漲到 ~6.4。15 個比較的
Bonferroni 門檻是 |t| ≈ 2.94，2.13 不過關。**雜訊。**

⚠ 順帶記錄一個容易誤讀的東西：基準線本身在 3個月/8幣 是 **−7.64R**，
在 6個月/15幣 是 **+37.59R**。**絕對數字完全被樣本選擇左右**，只有政策之間
的成對比較有效。看到「淨 +37R」不要當成策略會賺。

**現在四個層面（進場評分、出場管理、掛單機制、TP 位置）全部測完，沒有一個
測得出可調的槓桿。**

## 2026-09-01：Redis 恢復後首次漏斗判讀 → 只有熔斷站得住

`npm run funnel-verdict`（7 天窗口）。判定規則：**兩端同號才算數，而且先看
悲觀覆蓋率**——`29b2499` 之前結案的影子單永遠不會有悲觀值，它們的 netRPess
是 0，而 0 會讓「兩端同號」的判斷失效。

```
關卡                已結案  悲觀  覆蓋   樂觀netR  悲觀netR  判定
circuit_breaker       22    19   86%    -9.95    -9.95   ✅ 擋對了
score_gate            35    18   51%     9.18     1.01   覆蓋不足，不可用
no_entry_tf           28     7   25%     7.15     4.19   覆蓋不足，不可用
same_dir_cap          17     7   41%    13.30     6.50   覆蓋不足（容量型）
total_risk_cap        15     0    0%    24.16     0.00   覆蓋不足（容量型）
btc_chaos              8     0    0%     4.07     0.00   覆蓋不足，不可用
```

**八道關卡只有熔斷達標，判定確實在保護。其餘全部要再等。**

**撤回 8/25 的結論**：那份說「唯一有正面證據的是 `btc_chaos`（−5.82R）」。
重跑後 `btc_chaos` 是 **+4.07（符號翻面）且悲觀覆蓋 0%**——那 8 筆全是舊的
樂觀偏誤值。所以「邊際可能在風控濾網」目前**沒有資料支持**。

### 通過率 0.5%，而第二大關卡是我們知道訂太緊的那個

```
候選 790 → 發出 4（0.5%）
score_gate     205  26%
drawdown_halt  191  24%   ← 這道在 12R，實測純雜訊觸發率 24.9%
locked         119  15%
```

回撤停機一週擋掉 191 個候選，而 bootstrap 顯示 12R 有 1/4 機率純靠隨機波動
觸發。這是把門檻調到 p95≈18R 的直接理由。

### 時間止損（stall）是目前第二強的證據

```
stall   不砍 -13.45R   實際 +1.21R   → 省下 14.66R
```

跟 8/25 那次自我更正一致（「時間止損不該拿掉」）。這個不要動。

### ⚠ 兩個要當成量測假象的數字

`cancel_tp1_direct` 25勝0敗、`cancel_ran_away` 5勝0敗。真實資料裡全勝幾乎
不存在，多半是 `walkTpSl` 的樂觀偏誤或樣本太小。`funnel-verdict` 會自動標
出來，不要當成發現。
