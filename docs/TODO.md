...
## P0 — 量測誠實度（不修的話後面每個判斷都建立在錯資料上）

> 這兩項**不是策略調參，是修量測**。風險接近零（不動任何門檻/觸發邏輯），
> 但不修的話，之後每一份 CSV 分析都會像 8/6 那次一樣被捏造的標籤騙。
> 完整證據見 `docs/ANALYSIS-2026-08-12-量測污染與評分無鑑別力.md` §0。

### 0-A. `tier` 寫入層 `?? 'A'` 捏造標籤（跟 8/6 同一個 bug，只修了一半）
...
**沒有 `tier` 欄位**，所以每一筆策略B的單寫進 DB 時 `tier` 都是 `undefined`，
被 `?? 'A'` 補成 `'A'`。CSV 實證：4 筆 `得分` 14-17（策略B尺度）的單，
`級別` 全部寫著 `A級·1h`。

**8/6（`docs/ANALYSIS-2026-08-06B` §0）已經踩過這個坑一次**——當時查到
`page.tsx:924` 的 `?? 'A'` 讓 27 筆被誤標，整份分析結論翻盤。那次
`f467604` **只修了匯出層**，寫入層一路留到現在，所以資料庫裡存的本身就是
錯的，換匯出程式救不回來。

**怎麼修**：`?? 'A'` 拿掉，讓它寫 `null`（`tier` 欄位本來就 nullable）。
策略B要不要有自己的 tier 概念是另一個設計問題，**不要順手發明一個值填進去**
——寧可 null（誠實的「不知道」）也不要捏造。

**注意**：修完只影響新資料，**舊資料的錯誤標籤無法回溯修正**（DB 裡就是
'A'，分不出哪些是真 A 級哪些是策略B被補的）。可行的補救是用
`進場策略='B'` 這個可信欄位在分析時排除——但那只涵蓋 8/4 之後的資料。

### 0-B. `得分` 欄位混了兩套不相容的尺度

| 策略 | 分數範圍 | 門檻常數 |
|---|---|---|
| A（趨勢） | 60 ~ 77 | 基礎 40 + 五組上限 |
| B（均值回歸） | 14 ~ 17 | `MIN_SCORE_B = 10` |

同一個 `score` 欄位放兩套尺度，**任何跨策略的分數分桶都無意義**。
8/11 分析裡「低分桶 +0.41R 打敗高分桶 +0.224R、分數越高越差」那個嚇人的
結論，其實只是策略B兩筆大贏家（分數 14，+3.59R / +1.74R）被丟進「低分桶」
造成的假象——**清掉污染後根本沒有那個現象**（見 P1-C）。

**怎麼修**（擇一，需要討論）：
1. 分析層一律先照 `進場策略` 分組再看分數（最省，不動 schema，但每個讀取
   端都要記得做，容易再忘）。
2. 策略B 的分數正規化到跟策略A 同尺度後才寫入（改動大，且會讓 B 的
   歷史分數不可比）。
3. 新增 `score_scale` 欄位標明尺度（需 ALTER TABLE，最誠實）。
