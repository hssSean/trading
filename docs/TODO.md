...
### 0. Vercel Fluid Active CPU 超額（4h13m / 4h）

Hobby 方案近 30 天已經 106%，99.8% 來自本專案。每日曲線把因果講得很清楚：
7/20 掉到每天 1.5-2.5 分鐘（cron-job.org 排程失效）、7/27 跳回 8 分鐘
（排程重新啟用）。也就是 **每分鐘一輪的 cron ≈ 每天 8-9 分鐘 ≈ 每月 4-4.5 小時**，
而 App 本身的底噪只有每天約 2 分鐘（每月約 1 小時）。

已做：`1d1e4a1` 的兩項純浪費移除。**效果尚未量測**——部署時間是 7/28 下午。

**下一步（2026-07-29 起）**：看 7/29 這個完整日的長條。
- < 5 分鐘 → 夠了，本項關閉。
- ≈ 8 分鐘 → 做下面的 4H 快取。

**還沒做的最大一項：4H 純量快取**
`route.ts` 每輪對每個幣種抓 540 根 4H K 線，算 `calcAdx` + `calcAtrHistory`
+ ATR 百分位。但 4H K 線每 4 小時才變一次 —— 等於重複算 240 次，且 540 根佔
單一幣種抓取量的 44%。

做法傾向：**只快取「過去 539 根的 ATR 歷史」**（在同一根 4H 內是真正靜態的），
現值與 ADX 照樣每輪重算。這樣行為完全不變，但每輪只需要抓約 30 根。
用 module-scope 的記憶體快取即可（Fluid 會重用熱實例；冷啟動就重抓，無正確性風險），
不必動 Redis 指令額度。

**不要**直接把 regime/ADX 整包快取到 4H 邊界——ADX 有 23/18 遲滯帶，
快取住等於盤中不再更新 regime，那是行為變更不是最佳化。
